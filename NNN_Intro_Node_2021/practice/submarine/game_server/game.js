...
function addItem() {
  ...
  if (gameObj.itemsMap.has(itemKey)) {    // at (itemX, itemY), already exist ?
    return addItem();
  }
...
function addAir() {
  ...
  if (gameObj.airMap.has(airKey)) {       // at (airX, airY), already exist ?
    return addAir();
  }
...
function getMapData() {
  ...
  const playersAndNPCMap
    = new Map(Array.from(gameObj.playersMap).concat(Array.from(gameObj.NPCMap)));
...
function missileEmit(socketId, direction) {
  const playersAndNPCMap
    = new Map(Array.from(gameObj.playersMap).concat(Array.from(gameObj.NPCMap)));
    if (!playersAndNPCMap.has(socketId))
      return;
...
function checkGetItem(playersMap, itemsMap, airMap, flyingMissilesMap) {
  for (let [hashKey, playerObj] of playersMap) {
    ...
    for (let [missileId, flyingMissile] of flyingMissilesMap) {
      ...
      if (distanceObj.distanceX
               <= (gameObj.submarineImageWidth / 2 + gameObj.missileWidth / 2)
          && distanceObj.distanceY
                <= (gameObj.submarineImageWidth / 2 + gameObj.missileHeight / 2)
    ...
    switch (NPCObj.level) {
      case 1:
        if (Math.floor(Math.random() * 60) === 1) {
          NPCObj.direction
            = gameObj.directions[Math.floor(Math.random() * gameObj.directions.length)];
        }
